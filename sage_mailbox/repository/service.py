...
from sage_mailbox.models import (
    EmailMessage as DjangoEmailMessage,
    Attachment as DjangoAttachment,
    Flag as DjangoFlag,
    Mailbox as DjangoMailbox
)
...
class EmailSyncService:
        ...
        try:
            with IMAPClient(self.host, self.username, self.password) as client:
                with IMAPMailboxService(client) as mailbox:
                    ...
                    mailbox_obj, _ = DjangoMailbox.objects.get_or_create(name__contains=folder)

                    # Get the latest email date from the database
                    latest_email = DjangoEmailMessage.objects.filter(mailbox=mailbox_obj).order_by('-created_at').first()

                    # If there are already synced emails, fetch from the latest date
                    if latest_email:
                        criteria = IMAPSearchCriteria.since(latest_email.created_at.strftime("%d-%b-%Y"))
                    else:
                    ...
                    if msg_ids:
                        emails = mailbox.fetch(MessageSet(msg_ids), MessagePart.BODY_PEEK)
    ...
    def save_emails_to_db(self, emails, mailbox):
        ...
        for email in emails:
            created_email, created_attachments = self.create_or_update_email(email, mailbox)
            if created_email:
                created_emails_count += 1
            created_attachments_count += created_attachments

        return {"created_emails": created_emails_count, "created_attachments": created_attachments_count}
    ...
    def create_or_update_email(self, email: EmailMessage, mailbox):
        ...
        try:
            ...
            aware_date = make_aware(date_obj) if date_obj and date_obj.utcoffset() is None else date_obj
        ...
        # Handle attachments
        created_attachments_count = self.handle_attachments(django_email, email.attachments)
    ...
    def handle_attachments(self, django_email, attachments):
        ...
        for attachment in attachments:
            ...
            file_name = default_storage.save(f"attachments/{sanitized_filename}", file_content)
