...
from sage_mailbox.models import Attachment, EmailMessage, Sent, Trash, Junk
from sage_mailbox.models.mailbox import StandardMailboxNames, Mailbox
...
class AttachmentInline(admin.TabularInline):
    ...
    fields = ['filename', 'file', 'content_type', 'content_transfer_encoding',]
    readonly_fields = (
        'content_type',
...
class EmailMessageAdmin(admin.ModelAdmin):
    ...
    @admin.display(ordering='created_at', description=_('Recently Created'), empty_value=_("Not Recent"))
    def is_recent(self, obj):
        return obj.created_at >= datetime.now() - timedelta(days=7)

    @admin.display(boolean=True, ordering='has_attachments', description=_('Has Attachment'))
    ...
    def get_from_address(self, obj):
        return format_html('<div style="max-width: 130px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{}</div>', obj.from_address)

    @admin.display(ordering="message_id", description=_("Message-ID"))
    def get_message_id(self, obj):
        return format_html('<div style="max-width: 130px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{}</div>', obj.message_id)
    ...
    def get_urls(self) -> list[URLPattern]:
        urls = super().get_urls()
        custom_urls = [
            path('sync-emails/', self.admin_site.admin_view(self.sync_emails), name=f'{self.model._meta.app_label}_{self.model._meta.model_name}_sync'),
        ]
            ...
            try:
                ...
                with IMAPClient(host, username, password) as client:
                    with IMAPMailboxUIDService(client) as mailbox:
                        ...
                        logger.debug(f"Marked email with UID {email_message.uid} as SEEN on IMAP server.")

                        email_message.is_read = True
                        email_message.save(update_fields=['is_read'])
                        logger.debug(f"Marked email message object {email_message.pk} as read in the database.")
                        messages.success(request, _("Email marked as read."))
            except Exception as e:
                logger.error(f"Error marking email message {email_message.pk} as read: {str(e)}", exc_info=True)
                messages.error(request, _("Failed to mark the email as read on the IMAP server."))
        return super().change_view(request, object_id, form_url, extra_context)


@admin.register(Sent)
class SentAdmin(EmailMessageAdmin):
    mailbox_name = Mailbox.objects.get(folder_type=StandardMailboxNames.SENT).folder_type
    ...
    def get_queryset(self, request: HttpRequest):
        qs = super().get_queryset(request)
        return qs.select_related_mailbox().total_attachments().has_attachments().filter(mailbox__folder_type=StandardMailboxNames.SENT)

    def get_urls(self) -> list[URLPattern]:
        urls = super().get_urls()
        custom_urls = [
            path('sync-emails/sent/', self.admin_site.admin_view(self.sync_emails), name=f'{self.model._meta.app_label}_{self.model._meta.model_name}_sync'),
        ]
...
class JunkAdmin(EmailMessageAdmin):
    mailbox_name = Mailbox.objects.get(folder_type=StandardMailboxNames.SPAM).folder_type
    ...
    def get_urls(self) -> list[URLPattern]:
        urls = super().get_urls()
        custom_urls = [
            path('sync-emails/junk/', self.admin_site.admin_view(self.sync_emails), name=f'{self.model._meta.app_label}_{self.model._meta.model_name}_sync'),
        ]
        total = custom_urls + urls
        return total
...
class TrashAdmin(EmailMessageAdmin):
    mailbox_name = Mailbox.objects.get(folder_type=StandardMailboxNames.TRASH).folder_type
    ...
    def clear_trash(self, request: HttpRequest):
        ...
        try:
            ...
            with IMAPClient(host, username, password) as client:
                ...
                email_ids = mailbox_service.uid_search(IMAPSearchCriteria.ALL, charset=None)
                client.uid('STORE', MessageSet(email_ids).msg_ids, FlagCommand.ADD, Flag.DELETED)
            ...
            end_time = time.time()
            runtime = end_time - start_time
            self.message_user(request, _("Successfully permanently deleted emails in {:.2f} seconds.").format(runtime), messages.SUCCESS)

        except Exception as e:
            end_time = time.time()
            runtime = end_time - start_time
            logger.error(f"Error clearing trash: {str(e)}", exc_info=True)
            self.message_user(request, _("Failed to clear trash. Please try again. Task completed in {:.2f} seconds.").format(runtime), messages.ERROR)
